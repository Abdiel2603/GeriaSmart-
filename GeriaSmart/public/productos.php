...
<section class="centrado">
    <div class="container centrado-flex">
        ...
        <div class="centrado-text-col">
            <h1 class="reveal">Productos disponibles</h1>
            <p class="reveal" style="margin-bottom: 50px;">
                Aqui encontrarás nuestros productos disponibles, mira el que mejor se adapte a las necesidades que buscas.
            </p>
            ...
            <div class="p-4" style="padding: 10px">
                ...
                $active_products_exist = false;
                
                // Primero verificamos si hay productos activos
                foreach ($productos as $fila_tabla) {
                    if ($fila_tabla['estatus'] == 1) {
                        $active_products_exist = true;
                        break;
                    }
                }
                
                if (!$active_products_exist) {
                    echo '<h1 class="text-center py-4">No hay productos disponibles</h1>';
                } else {
                    ?>
                    <table class="table-striped reveal" style="border-radius: 30px; background: rgba(102, 102, 102, 0.11) !important; backdrop-filter: blur(15px) !important; box-shadow: 0 7px 20px rgba(0, 0, 0, 0.3);">
                        <thead>
                        <tr>
                            <th>Nombre</th>
                            <th>Modelo</th>
                            <th>Precio</th>
                            <th class="w-25">Vista</th>
                            <th></th>
                        </tr>
                        </thead>
                        <tbody>
                        ...
                        foreach ($productos as $fila_tabla) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <img class="img-fluid"
                                         src="<?= $fila_tabla['img'] ?>"
                                         alt="<?= $fila_tabla['nom_prod'] ?>">
                                </td>
                                <td class="btns" style="flex-direction: column; margin-top: 70px; gap: 5px">
                                    <form action="../lib/procesar_compra.php" method="POST" style="display: inline;">
                                        <input type="hidden" name="id_prod" value="<?= $fila_tabla['id_prod'] ?>">
                                        <button type="submit" class="btn btn-primary"
                                                <?= ($fila_tabla['stock'] <= 0) ? 'disabled' : '' ?>>
                                            <?= ($fila_tabla['stock'] <= 0) ? 'Sin stock' : 'Comprar' ?>
                                        </button>
                                    </form>

                                    <a class="btn-secondary" href="productodetalles.php?idprod=<?= $fila_tabla['id_prod'] ?>">Detalles</a>
                                </td>
                            </tr>
                            <?php
                        }
